...
const LOGO_GROUP_ORDER: LogoGroup[] = ["ai", "hosting", "payments", "streaming"]
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setCurrentGroup((prev) => {
        const currentIndex = LOGO_GROUP_ORDER.indexOf(prev)
        if (currentIndex === -1) return LOGO_GROUP_ORDER[0]!

        const nextIndex = (currentIndex + 1) % LOGO_GROUP_ORDER.length
        return LOGO_GROUP_ORDER[nextIndex]!
      })
    }, 2500)
